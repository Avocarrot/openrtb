...
Bid.prototype.replaceMacros = function(){
  return new Promise(function (resolve, reject) {
    if (!this.clearPrice){
      return reject(new Error('Cannot replace macros without a clear price'));
    }

    var valuesMap = { '${AUCTION_PRICE}': encodeURIComponent(this.clearPrice) };

    if (this.nurl) {
      this.nurl = replaceMacros(this.nurl, valuesMap);
    }

    if (this.adm){
      if (typeof this.adm === "object") {
        var replacedAdm = replaceMacros(JSON.stringify(this.adm), valuesMap);
        this.adm = JSON.parse(replacedAdm);
      } else {
        this.adm = replaceMacros(this.adm, valuesMap)
      }
...
BidBuilder.prototype.build = function() {
  return new Promise(function (resolve, reject) {
    if (!this._price){
      ...
      reject(new Error('Bid should have an nurl'));
    } else if (!this._adm){
      reject(new Error('Bid should have an adm'));
    } else {
      resolve(new Bid(
        this._status,
        this._nurl,

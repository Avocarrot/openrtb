...
var bidResponseSchema = require('./schemas/bidResponseSchema');
  ...
  return new Promise(function (resolve, reject) {
    if (!this._status){
      ...
      Promise.map(this._seatbid, function(sb){
        ...
        var validationResultArray = validator.validate(bidResponseSchema, bidResponse);

        if(validationResultArray.length === 0){
          resolve(bidResponse);
        } else {
          reject(new ValidationError(validationResultArray));
        }
